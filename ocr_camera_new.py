...
def run(camera_index=0):
    ...
    while True:
        ...
        # OCR loop (continuous, every frame)
        if ocr_started and regions:
            latest_texts = []
            for i, (x, y, w, h) in enumerate(regions):
                ...
                # Always write per-region file (overwrite every frame)
        ...
        if panel is not None:
            ...
            for i, text in enumerate(latest_texts):
                # Region header
                cv2.putText(panel, f"Region {i+1}:", (10, y0),
        ...
        if key == ord('q'):
            ...
            if not regions:
                ...
                print("OCR started (continuous, every frame).")
        elif key == ord('r'):
